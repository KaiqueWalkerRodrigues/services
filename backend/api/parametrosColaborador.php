...
$parametros = new ParametrosColaborador();

switch ($metodo) {
    case 'GET':
        if (!isset($_GET['id_filial_colaborador'])) {
            http_response_code(400);
            echo json_encode(["status" => "erro", "mensagem" => "O parâmetro id_filial_colaborador é obrigatório"]);
            break;
        }

        if ($loginValido) {
            echo json_encode($parametros->mostrar($_GET['id_filial_colaborador']));
        } else {
    ...
    case 'PUT':
        ...
        if ($loginValido) {
            echo json_encode($parametros->salvar(
                $data->id_filial_colaborador ?? null,
