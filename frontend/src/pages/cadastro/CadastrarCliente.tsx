  ...
  const handleFinalizarCadastro = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast({
          type: "error",
          message: "Erro ao processar o cadastro no servidor.",
          duration: 1250,
        });
      }
    } catch (error) {
...
function RegisterStep1({ dados, atualizarDados, onNext, onBack }: any) {
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="w-full rounded-2xl font-semibold hover:bg-white active:scale-[0.98] transition-all cursor-pointer bg-[#f4f4f4] text-black py-4 border-none text-[15px]"
      >
        Continuar
      </button>
    </form>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full bg-[#1c1c1c] text-white p-3.5 rounded-2xl outline-none transition-colors placeholder-[#6b7280] text-[15px] border ${erro ? "border-red-500" : "border-[#2a2a2a] focus:border-[#4a4a4a]"}`}
            />
          </div>
        </div>
        {erro && (
