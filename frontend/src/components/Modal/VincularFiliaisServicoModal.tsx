...
import { Building2, Check, Loader2, Settings2 } from "lucide-react";
import { ParametrosFilialServicoModal } from "./ParametrosFilialServicoModal";
...
export function VincularFiliaisServicoModal({
  ...
  const [filialParaConfigurar, setFilialParaConfigurar] =
  ...
  return (
    <>
      ...
      >
        <div className="space-y-4">
          <p className="text-xs text-zinc-400">
            Selecione abaixo quais filiais da empresa devem oferecer este
            serviço.
          </p>
                      ...
                      {processando ? (
                        ...
                        <>
                          {vinculada && (
                            <button
                              type="button"
                              onClick={() => setFilialParaConfigurar(filial)}
                              title="Configurar valores e durações específicas"
                              className="flex items-center justify-center h-8 w-8 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white transition-colors"
                            >
      ...
      <ParametrosFilialServicoModal
        isOpen={!!filialParaConfigurar}
        onClose={() => setFilialParaConfigurar(null)}
        onSuccess={onSuccess}
        servico={servico}
        filial={filialParaConfigurar}
      />
    </>
  );
}
